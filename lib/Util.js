            ...
            .then( weather => {
                ...
                new Jimp(450, 100, (err, image) => {
                    ...
                    .then( data => {
                        ...
                        let back;
                        if (timeOfDay == "night") {
                            back = resourceManager.getImage(`element_night`);
                        } else {
                            back = resourceManager.getImage(`${weatherState}_${timeOfDay}`);
                        }
                        ...
                        if (moonPhases.phase > 0 && moonPhases.phase < 0.25) { moonStage = 1; moonPhase = "Waxing Crescent"; }
                        if (moonPhases.phase == 0.25) { moonStage = 2; moonPhase = "First Quarter"; }
                        if (moonPhases.phase > 0.25 && moonPhases.phase < 0.5) { moonStage = 3; moonPhase = "Waxing Gibbous"; }
                        if (moonPhases.phase == 0.5) { moonStage = 4; moonPhase = "Full Moon"; }
                        if (moonPhases.phase > 0.5 && moonPhases.phase < 0.75) { moonStage = 5; moonPhase = "Waning Gibbous"; }
                        if (moonPhases.phase == 0.75) { moonStage = 6; moonPhase = "Last Quarter"; }
                        if (moonPhases.phase > 0.75) { moonStage = 7; moonPhase = "Waning Crescent"; }
                        
                        if (timeOfDay == "night") {
                            let moon = resourceManager.getImage("element_moon");
                            let rain = resourceManager.getImage("element_rain");
                            let cloud = resourceManager.getImage("element_clouds");
                            ...
                            // Add in the moon
                            moon.data.mask(mask.data, -(110 * moonStage), 0);
                            if (northenHemisphere == false) moon.data.flip(true, false);
                            image.composite(moon.data, 340, -15);

                            // Add in the clouds.
                            console.log(`Weather: ${weatherState}`);
                            if (weatherState == "light_rain" || weatherState == "light_cloud") {
                                cloud.data.opacity(0.75);
                                image.composite(cloud.data, 0, 0);
                            } else if (weatherState == "cloudy") {
                                cloud.data.opacity(0.25);
                                image.composite(cloud.data, 0, 0);
                            }

                            if (weatherState == "light_rain") {
                                image.composite(rain.data, 0, 0);
                            }
                        }
